<template>
	<div>
		<el-form :model="formData" :rules="rules" ref="ruleForm" size="mini" label-width="80px" label-position="left">
			...
			<el-form-item class="events"  prop="events">
        <label slot="label">服务事项</label>
			...
			<linkage @rule-check="checkLinkRule" @address-finish="getAddress"></linkage>
...
export default {
  ...
  methods: {
    ...
    submit (formName, formName1) {
      ...
      console.log(this.linkRule)
      this.linkRule.validate((valid) => {
        if (valid) {
          alert('suc')
        } else {
          console.log('error submit!!')
